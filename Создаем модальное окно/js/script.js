...
// Создаем модальное окно на сайте, т.е. когда при клике на кнопку "Узнать больше" в верстке появляется дополнительная информация в новом окошке
    let more        = document.querySelector('.more'),                // Получаем нашу кнопку "Узнать больше" из нашей верстве (она с классом more)
        overlay     = document.querySelector('.overlay'),             // Получаем наше модальное окно, которое должно показываться при клике на кнопку "Узнать больше". Там большой блок в верстве с классом overlay в конце
        close       = document.querySelector('.popup-close');         // Получаем так же кнопку в виде крестика, чтобы когда пользователь хотел выйти с этого модального окна, чтобы он попал обратно на сайт и наше модальное окно закрылось. Здесь мы получаем и эту кнопочку

    more.addEventListener('click', function () {                      // Берем нашу кнопку "Узнать больше" и навешиваем на нее обработчик события клика (т.е. при клике будет запускаться функция, которая реализована ниже)
        overlay.style.display = 'block';                              // При клике на кнопку наш overlay класс должен менять свои стили, а именно показываться, т.е. из display none переходить в display block, а для этого мы используем свойство style и меняем этот параметр переводя его в block
        this.classList.add('more-splash');                            // Конструкцию this используем, т.к. хотим показать, что при клике на кнопку "Узнать больше" мы хотим еще что-то делать. В данном случаем мы добавляем к нашей кнопке класс more-splash, который делает красивую анимацию подсвечивания нашей кнопки
        document.body.style.overflow = 'hidden';                      // Так же мы хотим запретить пользователю прокручивать страницу сайта пока активно модальное окно. Для этого есть такое css правило как overflow: hidden, с помощью которого это позволит нам запретить пользователю прокручивать страницу при активации модального окна
    });

    close.addEventListener('click', function () {                     // Здесь мы навешиваем обработчик события клика на наш крестик, чтобы закрыть наше модальное окно
        overlay.style.display = 'none';                               // Собственно здесь выполняется обратное действие, мы переводим наш overlay в состояние none, т.е. не видимое
        more.classList.remove('more-splash');                         // А здесь уже идет удаление класса more-splash, т.к. он не нужен когда не нажата клавиша "Узнать больше", соответственно нам надо удалить его. Но this мы уже использовать не можем, т.к. если бы мы использовали this, то обращались бы к клавише крестика, а у нас этот класс на кнопке "Узнать больше", собственно с ним и работаем
        document.body.style.overflow = '';                            // Здесь мы уже при выходе из модального окна, конечно же разрешаем пользователю снова прокручивать колесиком мышки сайт и все просматривать
    });

// Создаем модальное окно на сайте, т.е. когда при клике на кнопку "Узнать подробнее в табах" в верстке появляется дополнительная информация в новом окошке (рассшифровку кода не пишу, т.к. он похож с тем что выше)
    let btn       = document.querySelectorAll('.description-btn');

    for (let i = 0; i < btn.length; i++) {                            // Запускаем цикл, т.к. у нас кнопок этих 4 штуки в разных информационных табах, соответственно при проходе всех этих кнопок будет запускаться одна и та же функция с обработчиком
